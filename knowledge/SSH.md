...
## Overview
SSH (Secure Shell) is a network protocol for secure remote login, command execution, and file transfer over an untrusted network. It establishes an encrypted channel between client and server, protecting against eavesdropping and connection hijacking.

> [!note] Core guarantee
> SSH provides confidentiality, integrity, and authentication on top of a plain TCP connection — similar in spirit to TLS, but oriented around long-lived interactive sessions rather than request/response.
...
### 개요
SSH(Secure Shell)는 신뢰할 수 없는 네트워크에서 원격 로그인, 명령 실행, 파일 전송을 안전하게 수행하기 위한 프로토콜이다. 클라이언트와 서버 사이에 암호화된 채널을 만들어 도청과 연결 가로채기 같은 공격으로부터 통신을 보호한다.

> [!note] 핵심 보장
> SSH는 평문 TCP 연결 위에 기밀성, 무결성, 인증을 더해 준다. TLS와 비슷한 역할이지만, 요청/응답보다는 장시간 유지되는 인터랙티브 세션에 초점이 맞춰져 있다.
